...
<table class="tipodoc">
    <tr>
        <th colspan="4" >PRESUPUESTO CLIENTE</th>
    </tr>
    <tr>
         <td>SERIE</td>
         <td>NUMERO</td>
    ...
    <tr>
       <td>' . $presupuestoscliente['Presupuestoscliente']['serie'] . '</td>
       <td>' . zerofill($presupuestoscliente['Presupuestoscliente']['numero']) . '</td>
       <td>' . $this->Time->format('d-m-Y', $presupuestoscliente['Presupuestoscliente']['fecha']) . '</td>
       <td>' . '' . '</td>
    </tr>
    <tr>
...
$html .= '<p style="font-size: 0.8em">Observaciones: '.$presupuestoscliente['Presupuestoscliente']['observaciones'].'</p>';
...
foreach ($presupuestoscliente['Tareaspresupuestocliente'] as $tareaspresupuestocliente) {
    ...
    foreach ($tareaspresupuestocliente['Manodeobra'] as $manodeobra) {
        ...
        $html .= '<tr>';
        $html .= '<td style="text-align:  left;">DESPLAZAMIENTO ' . $tareaspresupuestocliente['TareaspresupuestoclientesOtrosservicio']['varios_descripcion'] . '</td>';
    ...
    if(!empty($tareaspresupuestocliente['Materiale'])):
        ...
        $html .= '<th style="text-align:  right;width: 12%;">PRECIO</th>';
        if ($hay_descuento) {
            $html .= '<th style="text-align:  right;width: 10%;">DTO.</th>';
            $html .= '<th style="text-align:  right;width: 15%;">IMPORTE</th>';
        } else {
            $html .= '<th style="text-align:  right;width: 23%;">IMPORTE</th>';
        }
        $html .= '</tr>';
        ...
        foreach ($tareaspresupuestocliente['Materiale'] as $materiale) {
            ...
            $html .= '<td style="text-align:  right;width: 12%;">' . number_to_comma($materiale['precio_unidad']) . ' €</td>';
            if ($hay_descuento) {
                ...
                $html .= '<td style="text-align:  right;width: 23%;">' . number_to_comma($materiale['importe']) . ' €</td>';
            }
...
$pdf->Output('presupuestocliente.pdf', 'I');
?>
